...
if [ ! -d "node_modules" ]; then
  if command -v npm >/dev/null 2>&1; then
    ...
    echo "  [!] npm command not found. Please install npm."
    echo ""
  fi
fi
...
if command -v python3 >/dev/null 2>&1; then
  ...
  if python3 -c "import curl_cffi" >/dev/null 2>&1; then
    echo "  [+] Cloudflare bypass engine: Active (curl-cffi Chrome 124)"
  fi
fi
...
if kill -0 $SERVER_PID 2>/dev/null; then
  ...
  echo "  [ERROR] Failed to start AniStream server. Check $LOG_FILE for details."
  cat "$LOG_FILE"
  exit 1
fi
